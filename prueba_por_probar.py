...
from tkinter import font, simpledialog, messagebox, filedialog, ttk
import socket
import threading
import os
import time
...
class ChatGUI:
    ...
    def create_title(self):
        self.title_label = tk.Label(self.master, text="AnonymChat", font=("Arial", 36, "bold"), bg=self.master.cget("bg"), fg=self.text_fg)
    ...
    def create_menu(self):
        ...
        settings_menu.add_command(label="Cambiar nombre de usuario", command=self.change_username)
        settings_menu.add_command(label="Configuración de red", command=self.network_settings)
        settings_menu.add_separator()
        settings_menu.add_command(label="Salir", command=self.close_application)
        menubar.add_cascade(label="Ajustes", menu=settings_menu)
        self.master.config(menu=menubar)

    def change_username(self):
        new_username = simpledialog.askstring("Cambiar nombre de usuario", "Ingresa tu nuevo nombre de usuario:")
        if new_username:
            self.current_user = new_username

    def network_settings(self):
        messagebox.showinfo("Configuración de red", "Actualmente no hay configuraciones adicionales disponibles.")
    ...
    def enviar_archivo(self):
        ...
        if file_path and self.client_socket:
            try:
                ...
                self.client_socket.send(f"Archivo: {file_name}".encode())
                with open(file_path, "rb") as file:
                    self.client_socket.sendall(file.read())
                messagebox.showinfo("Archivo Enviado", f"El archivo {file_name} se ha enviado con éxito.")
            except Exception as e:
    ...
    def handle_client(self, client_socket, address):
        ...
        if message.startswith("Solicitud de chat"):
            ...
            with open(file_name, "wb") as file:
                while True:
                    ...
                    if not data:
                        break
                    file.write(data)
            messagebox.showinfo("Archivo recibido", f"Has recibido un archivo: {file_name}")
